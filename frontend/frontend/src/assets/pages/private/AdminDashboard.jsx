...
const AdminDashboard = () => {
  ...
  const handleUpdateReportStatus = async () => {
    ...
    try {
      ...
      toast.success(`Report status updated to ${reportStatus}`);
  ...
  const handleRespondToIssue = async () => {
    ...
    if (!issueResponse || issueResponse.trim().length < 10) {
      toast.warning('Please provide a response (at least 10 characters)');
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to respond to issue');
      setMessage({
        type: 'error',
  ...
  const handleUnsuspendUser = async () => {
    ...
    try {
      ...
      toast.success(`User ${selectedUser.username} has been reactivated`);
  ...
  const handleCancelRide = async () => {
    ...
    try {
      ...
      toast.success(`Ride #${selectedRide.id} has been cancelled`);
  ...
  const handleDeleteRide = async () => {
    ...
    try {
      ...
      toast.success(`Ride #${selectedRide.id} has been deleted`);
  ...
  const handleApprove = async () => {
    ...
    if (approvalType === 'rider' && !hasLicense) {
      toast.warning('Cannot approve as rider without driving license documents');
      setMessage({ type: 'error', text: 'Cannot approve as rider without driving license documents' });
      return;
    }
    
    if (approvalType === 'user' && !hasCitizenship) {
      toast.warning('Cannot approve as user without citizenship documents');
    ...
    try {
      ...
      toast.success(`Verification approved as ${approvalType}!`);
