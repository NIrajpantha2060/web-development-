...
export const rideAPI = {
  ...
  // Delete ride from history (soft delete)
  deleteRideFromHistory: async (rideId) => {
    const token = localStorage.getItem('token');
    const response = await axios.delete(`${API_BASE_URL}/rides/history/${rideId}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
  }
...
export const bookingAPI = {
  ...
  // Delete booking from history (soft delete)
  deleteBookingFromHistory: async (bookingId) => {
    const token = localStorage.getItem('token');
    const response = await axios.delete(`${API_BASE_URL}/bookings/history/${bookingId}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    return response.data;
  }
